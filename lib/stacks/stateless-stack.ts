...
export class StatelessResourceStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: StatelessResourceProps) {
    ...
    /* ---------------------------------------------------------------------
       Wire service to BOTH target groups
       (populates the "LoadBalancers" array that CFN needs)
    --------------------------------------------------------------------- */
    // Wire the Fargate service to both target groups (blue = prod, green = stage)
    this.backendService.attachToApplicationTargetGroup(blueTG);
    this.backendService.attachToApplicationTargetGroup(greenTG);
